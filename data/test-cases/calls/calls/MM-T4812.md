...
Create 2 of your own users on the test server - users + UserB.\
\
...
\
\*\*Professional\*\*\
\- channels - start a call with 7 users using the load test command -> join call with users -> verify that UserB cannot join and see the appropriate UI changes -> leave with users -> verify that UserB can now join\
\- DM - can start and join calls\
\- GM - can start and join calls\
\
\*\*Enterprise\*\*:\
