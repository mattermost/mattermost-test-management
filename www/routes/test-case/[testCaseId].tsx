import tcKeyAndPath from "../../../data/key-and-path.json" assert { type: "json" };
...
export const handler = {
    GET(_req, ctx) {
      ...
      let foundEntry = keyAndPath.filter(it => it.key.toUpperCase() === testCaseId);
      ...
      if (foundEntry.length !== 0) {
        return new Response("", {
          status: 301,
          headers: { location: `/test-cases/${foundEntry[0].path}/${foundEntry[0].key.toLowerCase()}` },
        });
      } else {
